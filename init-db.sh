...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE DATABASE ${USER_DB_NAME} IF NOT EXISTS;
    CREATE USER ${USER_DB_USER} WITH PASSWORD '${USER_DB_PASSWORD}' IF NOT EXISTS;
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE DATABASE ${KANBAN_DB_NAME} IF NOT EXISTS;
    CREATE USER ${KANBAN_DB_USER} WITH PASSWORD '${KANBAN_DB_PASSWORD}' IF NOT EXISTS;
